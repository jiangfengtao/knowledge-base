 ...
 * 
 * 安全：仅在数据库未初始化时可调用，或通过 secret 参数验证
 */
export async function POST(request: Request) {
  try {
    ...
    if (!user) {
      ...
      results.push(`Created user: ${user.name} (${user.id})`);
    } else {
      results.push(`User exists: ${user.name}`);
    }
    ...
    for (const kb of defaultKbs) {
      let existing = await prisma.knowledgeBase.findFirst({
        where: { name: kb.name, userId },
      });
      if (!existing) {
    ...
    results.push("Default KBs created");
    ...
    let mediaKb = await prisma.knowledgeBase.findFirst({
      where: { name: { contains: "自媒体" }, userId },
    });

    if (!mediaKb && kb02) {
      mediaKb = await prisma.knowledgeBase.create({
        data: {
          name: "晓桃自媒体运营",
          icon: "🎯",
          parentId: kb02.id,
      ...
      results.push("Created media KB");
    } else {
      results.push("Media KB exists");
    }

    if (!mediaKb) {
      return NextResponse.json(
        { success: false, error: "Failed to create media KB" },
        { status: 500 }
      );
    }

    // === 4. 创建自媒体知识库分类和文档 ===
    await seedMediaKnowledge(userId, mediaKb.id, results);
...
async function seedMediaKnowledge(userId: string, mediaRootId: string, results: string[]) {
  ...
  const findOrCreateTag = async (name: string, color: string = "#4ECDC4") => {
    let tag = await prisma.tag.findFirst({
      where: { name, userId, parentId: null },
    });
    if (!tag) {
  ...
  const findOrCreateKb = async (name: string, parentId: string, opts: any = {}) => {
  ...
  const findOrCreateDoc = async (title: string, kbId: string, content: string, tags: string[] = []) => {
    let doc = await prisma.document.findFirst({
      where: { title, knowledgeBaseId: kbId, userId },
    });
    if (!doc) {
      const plainText = content.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
      doc = await prisma.document.create({
        data: { title, content, plainText, wordCount: plainText.length, knowledgeBaseId: kbId, userId },
      });
  ...
  for (const cat of categories) {
    const parent = await findOrCreateKb(cat.name, mediaRootId, { icon: cat.icon, color: cat.color, sortOrder: sortOrder++ });
    kbMap[cat.name] = parent.id;

    let childSort = 0;
    for (const child of cat.children || []) {
      const childKb = await findOrCreateKb(child.name, parent.id, { icon: child.icon, sortOrder: childSort++ });
  ...
  for (const doc of docs) {
    ...
    if (kbId) {
      await findOrCreateDoc(doc.title, kbId, doc.content, doc.tags);
      docCount++;
    }
  }
  results.push(`Created ${docCount} documents`);
  ...
  for (const note of notes) {
    const existing = await prisma.note.findFirst({
      where: { content: note.content, userId },
    });
    if (!existing) {
  ...
  results.push(`Created ${noteCount} notes`);
}
